...
**No verificado con una sesión real en el browser** (JUICIO, declarado a propósito): esta app usa el proyecto de Supabase real de la familia, con compras y hogar reales ya cargados — crear una cuenta de prueba para verificar visualmente el splash hubiera significado sumar un usuario/hogar de basura a la base de datos real en producción, algo que se evitó a propósito. La verificación se hizo por otra vía: lectura directa y citada de la documentación de Next.js que describe exactamente este mecanismo, más `npm run build` y `npm run lint` limpios (confirman que el árbol de Suspense compila y tipa bien). Si en el uso real se nota que el splash no aparece donde se espera, es el próximo punto a revisar con datos reales de uso, no con una cuenta de prueba.
